...
def heuristicValue(grid):
        ...
        w2 = 2
        w3 = 3
        return w1*heuristic1(grid) + w2*heuristic2(grid) + w3*heuristic3(grid)
    
def heuristic1(grid):
    #Monotonic Rows -- max heuristic is 1
        hscore1 = 0
        hscore2 = 0
        for row in grid.map:
            sorted_row = row[:]
        ...
        return (500 - hscore2)/500
            
def heuristic2(grid):
    #Empty spaces -- max heuristic is 1
        return len(grid.getAvailableCells())/16
def heuristic3(grid):
    #Gives a bonus if the largest tile is in a corner
    max_tile = grid.getMaxTile()
    if  max_tile == grid.map[0][0]:
        return 1
    else:
        return 0
