# -*- coding: utf-8 -*-
"""Image Captioning NLP HW5.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/12UUQF03XG7aWA3speFQFdE5eg6Spp7ZD
...
image = PIL.Image.open(os.path.join("gdrive/My Drive/",IMG_PATH, dev_list[20]))
image
...
def get_image(image_name):
    image = PIL.Image.open(os.path.join('gdrive/My Drive/',my_data_dir, IMG_PATH, image_name))
...
def img_generator(img_list):
    ...
    for img in img_list:
      new_img = np.array([get_image(img)])
      yield new_img
...
def read_image_descriptions(filename):    
    ...
    while True:
      ...
      image_descriptions[line[0].split("#")[0]].append(description)
...
desc = read_image_descriptions("gdrive/My Drive/"+my_data_dir+"/Flickr8k.token.txt")
file = open("gdrive/My Drive/"+my_data_dir+"/Flickr_8k.trainImages.txt")
...
for img in images:
  if img in desc:
    ...
    print(img)
words = set(["<START>", "<END>"])
for value in desc2.values():
  for sentence in value:
    new_words = set(sentence)
...
def text_training_generator(batch_size=128):
  ...
  while True:
    ...
    for img in L:
      for sentence in descriptions[img]:
        for i in range(len(sentence)):
          count += 1
          if count == batch_size:
            yield (np.array(input_array), np.array(output_array))
            input_array = []
            output_array = []
            count = 0
          else:
            vec = np.zeros(40, dtype = int)
            for j in range(i):
              vec[j] = word_to_id[sentence[j]]
            if i == 0:
              continue
            input_array.append(vec)
            temp = np.zeros(vocab_size, dtype = int)
            try:
              temp[word_to_id[sentence[j+1]]] = 1
...
def decoder():
  prediction = ["<START>"]
  for i in range(39):
    input_array = np.zeros(40, dtype = int)
    for i in range(len(prediction)):
      input_array[i] = word_to_id[prediction[i]]
    pred = model.predict(np.array([input_array]))
    val = max(list(pred[0]))
    prediction.append(id_to_word[list(pred[0]).index(val)])
    if id_to_word[list(pred[0]).index(val)] == "<END>":
      return prediction
...
def sample_decoder():
  ...
  for i in range(39):
    ...
    pred = model.predict(np.array([input_array]))
    if sum(pred[0]) > 1:
      pred[0] = [i / 1.001 for i in pred[0]]
    val = np.random.multinomial(1, pred[0])
    val = list(val).index(1)
    prediction.append(id_to_word[val])
    if id_to_word[val] == "<END>":
      return prediction
  prediction.append("<END>")
  return prediction
    ...
    for img in L:
      for sentence in descriptions[img]:
        for i in range(len(sentence)):
          count += 1
          if count == batch_size:
            yield ([np.array(input_img), np.array(input_vec)], np.array(output_array))
            input_vec = []
            input_img = []
            output_array = []
            count = 0
            ...
            if i == 0:
              continue
            input_img.append(enc_train[train_list.index(img)])
...
def image_decoder(enc_image): 
    ...
    prediction[0] = word_to_id["<START>"]
    for i in range(1, 40):
      pred = model.predict(([[enc_image],[prediction]]))
      prediction[i] = list(pred[0]).index(max(pred[0]))
      if prediction[i] == word_to_id["<END>"]:
        return [id_to_word[j] for j in prediction[:i+1]]
    prediction[i] = word_to_id["<END>"]
    return [id_to_word[k] for k in prediction]
...
def img_beam_decoder(n, image_enc):
  ...
  try:
      enc_image = enc_train[train_list.index(image_enc)]
  except ValueError:
      enc_image = enc_dev[dev_list.index(image_enc)]

  for i in range(n):
    seq = np.zeros(40 , dtype = int)
    seq[0] = word_to_id["<START>"]
    beam.append((1, seq))

  for j in range(1, 40):
    newbeam = []
    try:
      beam = sorted(beam, reverse = True)[:n]
    except ValueError:
      beam = beam[:n]
    for tup in beam:
      if end in tup[1]:
        newbeam.append(tup)
        continue
      pred = model.predict(([[enc_image], [tup[1]]]))
      probs = sorted(list(pred[0]), reverse = True)[:n]
      for k in range(n):
        newseq = tup[1].copy()
        newseq[j] = list(pred[0]).index(probs[k])
        newtup = (tup[0]*probs[k], newseq)
        newbeam.append(newtup)
    beam = newbeam[:]
  best_output = sorted(beam, reverse = True)[0][1]
  best_output = best_output[ :list(best_output).index(59) + 1]
  return [id_to_word[a] for a in  best_output]

plt.imshow(get_image(dev_list[1]))
print(img_beam_decoder(3, dev_list[1]))
...
for i in range(5):
  plt.imshow(get_image(dev_list[i]))
  plt.show()
  print("Greedy output:", image_decoder(enc_dev[i]))
  print("Beam search n = 3 output:",img_beam_decoder(3, dev_list[i]))
  print("Beam search n = 5 output:", img_beam_decoder(5, dev_list[i]))