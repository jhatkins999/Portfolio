...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        rep = np.zeros(6)
        for i, index in zip([-1,-2,-3], [0,1,2]):
            try:
                if pos[state.stack[i]] in ["CD","NNP","UNK","ROOT","NULL"]:
                    rep[index] = self.word_vocab["<"+pos[state.stack[i]]+">"]
                elif pos[state.stack[i]] == None:
                    rep[index] = self.word_vocab["<ROOT>"]
                else:
                    try:
                        rep[index] = self.word_vocab[words[state.stack[i]].lower()]
                    except KeyError:
                        rep[index] = 2
            except IndexError:
                rep[index] = 4         
        for j, index2 in zip([-1,-2,-3], [3,4,5]):
            try:
                if pos[state.buffer[j]] in ["CD","NNP","UNK","ROOT","NULL"]:
                    rep[index2] = self.word_vocab["<"+pos[state.buffer[j]]+">"]
                elif pos[state.buffer[j]] == None:
                    rep[index2] = self.word_vocab["<ROOT>"]
                else:
                    try:
                        rep[index2] = self.word_vocab[words[state.buffer[j]].lower()]
                    except KeyError:
                        rep[index2] = 2
            except IndexError:
                rep[index2] = 4  
